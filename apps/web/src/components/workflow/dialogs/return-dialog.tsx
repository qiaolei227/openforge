...
interface Props {
  open: boolean;
  /** 'prev' = return to previous node; 'start' = return to start (submitter). */
  mode: 'prev' | 'start';
...
export function ReturnDialog({
  ...
  const title =
    mode === 'prev'
      ? tDialog('returnPrevTitle')
      : tDialog('returnStartTitle');
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await workflowTaskApi.returnTask(taskId, mode, trimmed);
