...
export function RejectDialog({ open, onOpenChange, taskId, onDone }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await workflowTaskApi.reject(taskId, trimmed);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{tDialog('rejectTitle')}</DialogTitle>
        </DialogHeader>
