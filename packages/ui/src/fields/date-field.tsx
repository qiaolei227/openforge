...
export default function DateField({ field, value, onChange, disabled, error, mode }: FieldComponentProps) {
  const inputRef = useRef<HTMLInputElement>(null);

  if (mode === 'view') {
    if (!value) return <span className="text-sm">{'\u2014'}</span>;
    try {
      return <span className="text-sm">{format(parseISO(value), 'yyyy-MM-dd')}</span>;
    } catch {
  ...
  const openPicker = () => {
    ...
    if (!el) return;
    // showPicker is supported in Chromium/Edge/Safari 16+; fall back to focus()
  ...
  return (
    <div>
      <div
        className={`${WRAPPER_BASE} ${error ? 'border-red-500' : ''} ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
      >
        <input
          ref={inputRef}
          type="date"
